...
**Conversation Buffer Memory**는 모든 대화를 그대로 저장하여 간단하지만 토큰 소모가 크다. **Summary Memory**는 대화를 주기적으로 요약하여 토큰을 절약한다. **벡터 메모리**는 임베딩으로 저장하고 관련성 기반으로 검색하여 장기 메모리를 효율적으로 관리한다. lumi-agent에서 Supabase를 장기 메모리 저장소로 활용한다.
