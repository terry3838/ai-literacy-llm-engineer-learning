...
LangGraph는 단일 에이전트부터 멀티 에이전트 시스템까지 구현 가능하다. Supervisor 패턴으로 여러 서브 에이전트를 조율하거나, 병렬 실행 브랜치를 통해 효율적인 작업 분배가 가능하다. lumi-agent 프로젝트(Week08~09)에서 핵심 프레임워크로 사용되었다.
